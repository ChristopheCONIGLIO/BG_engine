...
class exp5 extends BG_script{
    ...
    constructor(bg,decX,decY,layer) {
        ...
        // affichage text
        this.text = undefined;
        this.text = new BG_text(bg,true,false,this.p_layer+1,this.decX+20,this.decY+20,18,"#FFFFFF");
        this.text.setText("Experience 5: ");
    ...
    enterFrame(){
        ...
        for(var j = 0 ; j < this.arrObj.length ;j++){
            if( this.arrObj[j].getPosY() - this.decY > 550){
                this.arrObj[j].remove();
                this.arrObj[j] = undefined;
            }
            else if( this.arrObj[j].getPosY() - this.decY < -50){
                this.arrObj[j].remove();
                this.arrObj[j] = undefined;
            }
        }
