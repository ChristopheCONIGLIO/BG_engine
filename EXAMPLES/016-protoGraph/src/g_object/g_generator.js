...
class g_generator extends g_prototype{
    ...
    constructor(bg,decX,decY,layer,size) {
        ...
        this.p_counterGenerator = 0;
        this.p_counterGeneratorLimit = 30;
        ...
        this.cicleTestGreen = new BG_circleBorder(this._bg,true,false,
                                    ...
                                    this.p_sizeBordure,this.colorBorder,false);
    ...
    enterFrameMode0(){
        ...
        if( terrain_free && this.p_bg.bg_g_stat.getMouseClick() == 1 ){
            ...
            this.cicleTestGreen.pCrossable(true);
    ...
    enterFrameMode10(){
        ...
        this.p_counterGenerator++;
        if( this.p_counterGenerator>=this.p_counterGeneratorLimit){
            // TODO pTerrain
            var posX = this.cicleTestGreen.p_pX+this.cicleTestGreen.getDim()/4+Math.random()*this.cicleTestGreen.getDim()/2;
            var posY = this.cicleTestGreen.p_pY+this.cicleTestGreen.getDim()/4+Math.random()*this.cicleTestGreen.getDim()/2;
            
            var terrainFree = this._bg.pTerrainFreeCrossable(
                            posX,
                            posY,
                            34);
            if( terrainFree ){
                var node = new g_particule_gravity(this._bg,posX,posY,8,0,1);
                node.setMode10(posX,posY);
                this.p_counterGenerator = 0;
                // on s'en fou de la ref car particule est autonmae
            }
